        ...
        <form action="{{ route('dashboard.customers.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 max-w-6xl gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="vehicle_type" name="vehicle_type" value="{{ old('vehicle_type') }}" required />
                    <x-input-error :messages="$errors->get('vehicle_type')" class="mt-1" />
                </div>

                <div>
                    <x-input-label for="vehicle_plate" value="Plat Nomor" />
                    <x-text-input id="vehicle_plate" name="vehicle_plate" value="{{ old('vehicle_plate') }}" required />
                    <x-input-error :messages="$errors->get('vehicle_plate')" class="mt-1" />
                </div>

                <!-- Alamat jadi 1 kolom penuh -->
